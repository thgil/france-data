...
- Hook: France has 35,014 communes. One is called Y. Twelve are called Sainte-Colombe. 4,443 have "saint" somewhere in their name.
- Data: Commune names derived from DREES APL 2023 commune list (35,014 communes). No new data download required — existing communes-apl.geojson contains communeName field.
- Angle: Text-driven data story with inline bar chart (most common names), histogram (name length distribution), and highlighted extremes (shortest, longest, most common).
- Draft X summary:
  > France has 35,014 communes. One is called Y. Twelve are called Sainte-Colombe. 4,443 have "saint" somewhere in their name. The longest name is 45 characters. Today on **france-data**: what's in a name. 🇫🇷
