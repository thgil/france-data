...
### DS-COMMUNES-INDEX: Commune names and centroids (derived)
- URL: N/A — derived from DS-APL and DS-CONTOURS
- Publisher: Derived dataset (france-data project)
- Coverage: 35,014 French communes with INSEE code, name, département code, and approximate centroid (lat/lng).
- Formats: JSON
- Notes: Centroids are computed as the mean of polygon vertex coordinates from the DS-CONTOURS 1000m-simplified polygons — good enough for dot-density and scatter maps, not for precise boundary work. Stored at stories/commune-names/communes-index.json (3 MB). Population field (pop) is from DS-APL INSEE 2021 estimates.
