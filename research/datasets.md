...
### DS-COMMUNES-INDEX: Communes index (names + centroids, all France)
- URL: derived from DS-APL + DS-CONTOURS; no external download required
- Publisher: INSEE / DREES (via communes-apl.geojson)
- Coverage: 35,014 communes including overseas territories. Commune names, INSEE codes, département codes, approximate polygon centroids.
- Formats: JSON (2.7 MB)
- Notes: Lightweight extract for stories that need commune names or codes but not full geometry. Located at stories/commune-names/communes-index.json. Generated by averaging polygon ring coordinates — centroids are approximate but accurate to ~1 km for most communes.
