...
### DS-COMMUNES-INDEX: communes-index.json (derived)
- URL: (derived from DS-APL + DS-CONTOURS — no separate download)
- Publisher: Derived
- Coverage: 35,014 communes including overseas territories. Names from DREES APL 2023; centroids computed from Etalab contours 2025 polygon mean-vertex arithmetic centroid.
- Formats: JSON (3.1 MB). Fields: code (INSEE 5-digit), name (communeName), dept, pop, lat, lng.
- Notes: The centroid is a simple arithmetic mean of polygon vertex coordinates — adequate for story-level dot placement, not for precision navigation. Stored at stories/commune-names/communes-index.json for reuse by future stories that need commune-level lookup without loading the full 10 MB GeoJSON.
