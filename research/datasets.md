...
### DS-COMMUNES-INDEX: communes-index.json (all-France commune name index)
- URL: derived from DS-APL + DS-CONTOURS (no separate download)
- Publisher: derived dataset
- Coverage: 35,014 communes (metropolitan France + overseas territories). 2025 vintage.
- Formats: JSON (3.0 MB), fields: name, code, dept, pop, lat, lng (centroid approximation)
- Notes: Compact index derived by extracting communeName, code, dept, pop from communes-apl.geojson and computing ring-average centroids. Suitable for name analysis and lightweight lookups; not a substitute for the full polygon GeoJSON for choropleth rendering.
