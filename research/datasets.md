...
### DS-COMMUNES-INDEX: Communes index with names, codes, centroids
- URL: (derived from DS-APL + DS-CONTOURS; no external download needed)
- Publisher: derived from DREES / INSEE
- Coverage: 35,014 French communes (metropolitan + overseas), 2023 vintage
- Formats: JSON (stories/commune-names/communes-index.json, 3 MB)
- Notes: Lightweight lookup table with fields: name (communeName), code (INSEE 5-digit), dept, pop (INSEE 2021), lon/lat (centroid approximated from polygon bounding box centroid). Created for the commune-names story; reusable for any story needing a commune name/code lookup. Does NOT include full polygon geometry — use DS-CONTOURS for that.
